...
def recover():
    ...
    with open('index/index.txt', 'rb') as f:
        while True:
            str = f.read(12)
            if str == "":
                break
            ...
            for i in range(df): # how many docs
                ...
                index[term][doc_number] = pos_arr

def analyze_query(line, tokenize_regex):
    query_no = line.split(".")[0]
    terms_part = line[line.find(".")+1:]
    terms = re.findall(tokenize_regex, terms_part)
    return query_no, [term.lower() for term in terms[3:]]


def compute_scores(query_no, query_terms):
    global index, df_counts, ttf_counts, doc_lens, avg_length, total_docs, voc_size
    okapi_tf_file = 'results2/okapi_tf.txt'
    okapi_bm25_file = 'results2/okapi_bm25.txt'
    unigram_lm_laplace_file = 'results2/unigram_lm_laplace.txt'
    ...
    for q_term in query_terms:
        if not index.has_key(q_term):
            continue
        print q_term,
        for doc_number in doc_lens.keys():
            doc_len = doc_lens[doc_number]
            if index[q_term].has_key(doc_number):
                pos_arr = index[q_term][doc_number]
                tf = len(pos_arr)
            else:
                tf = 0
...
def remove_previous_results():
    dir = os.path.dirname(__file__) + "/results2"
    ...
    with open(query_file) as f:
        for line in f:
            if not line.startswith("\n"):
                query_no, terms = analyze_query(line, tokenize_regex)
                print "\nquery_no: " + query_no+" :",
                compute_scores(query_no, terms)